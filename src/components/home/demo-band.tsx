...
export function DemoBand({ video }: { video: DemoVideo }) {
  return (
    <section className="home-band home-video-band home-video-band--overview" data-testid="overview-video" id="overview-video">
      <div className="home-band__inner home-video-band__inner home-video-band__inner--overview">
        <header className="home-video-band__copy" data-reveal>
          <p className="home-eyebrow">FULL PRODUCT DEMO</p>
          <h2>{video.title}</h2>
          <p>{video.summary}</p>
        ...
        <div data-reveal><DemoVideoPlayer video={video} /></div>
      </div>
    </section>
  );
}
